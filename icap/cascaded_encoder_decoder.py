...
class ImageCaptioningWithCascadedEncoderDecoder(ImageCaptioning):
    def __init__(self, dataset_name=None, deployment=False):
    ...
    def predict(self, image_id=None, file_path=None):
        ...
        if file_path:
            image_features = self.image_feature_extractor.get_feature(file_path=file_path)
        else:
            image_features = self.image_feature_extractor.get_feature(image_id=image_id)
