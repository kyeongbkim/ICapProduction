...
class ImageCaptioningWithTransformer(ImageCaptioning):
    def __init__(self, dataset_name=None, batch_size=64, deployment=False):
    ...
    def data_loader(self, caption_data, batch_size):
        while True:
            ...
            for image_id, caption_id in caption_data.shuffled_train_data:
                image_feature = self.image_feature_extractor.get_feature(image_id=image_id)
    ...
    def predict(self, image_id=None, file_path=None):
        vocab = self.vocab
        max_length = 100

        if file_path:
            img_tensor = np.array([ self.image_feature_extractor.get_feature(file_path=file_path) ])
        else:
            img_tensor = np.array([ self.image_feature_extractor.get_feature(image_id=image_id) ])
